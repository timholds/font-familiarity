...
## Embedding Collapse Issue (V0.5.0)

We've identified a critical mode collapse in the font embeddings generated by our character-based model. Analysis of the cosine similarity distribution between all pairs of class embeddings reveals:

![Cosine Similarities Histogram](cosine_similarities_histogram_512d.png)

### Key Findings:
- **Catastrophic collapse**: ~20-25% of font pairs have cosine similarity = 1.0 (identical embeddings)
- **Bimodal distribution**: Main cluster at -0.260 (good separation) but massive spike at 1.0
- **Missing middle ground**: Almost no similarities in [0.25, 0.75] range

### Likely Causes:
1. **Aggregation failure**: Self-attention mechanism collapsing diverse character features
2. **Over-regularization**: Weight decay at .005 compared to learning rate at .00001 is destroying discriminative features  
3. **Missing contrastive objective**: Cross-entropy alone insufficient to prevent collapse
4. **Data quality**: Possible duplicate fonts or inconsistent CRAFT detections

### Proposed Solutions:
- Add contrastive/triplet loss to explicitly separate different fonts
- Reduce dropout rates (especially in attention: 0.1 or less)
- Add diversity penalty or spectral normalization
- Investigate which specific font classes are collapsing to identical embeddings  


# Issue with Flip flopping predictions in v5 models
When we test the model at epoch 8, 9, and 10, which is after the model has converged according to the loss curves. Looking at the actual top 5 predicted fonts shows that the model is flip flopping between its predictions. This indicates that the learning rate is too high or there is too much regularization, causing the model to oscillate around a solution instead of converging to a stable one.

# A note on model saving Footguns
-  v4 models: Saved with torch.save({'model_state_dict': model.state_dict(), ...})
- v5+ models: Saved with torch.save({'model': model, ...}) - stores the ENTIRE model object

When loading a full model object, PyTorch doesn't save the actual code - it saves references to class names (like "char_model.SelfAttentionAggregator"). On load, it uses the CURRENT code definitions. This is why v5 models break - they were saved with the old SelfAttentionAggregator (no dropout) but load with the new one (expects dropout).
